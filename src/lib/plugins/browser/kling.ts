...
const AUTOMATION   = path.join(process.cwd(), 'src', 'lib', 'plugins', 'browser', 'automate-kling.ts')

/**
 * Browser automation: drives kling.ai free web interface.
 * Setup: npm run browser:setup:kling
 * Activate: VIDEO_GENERATOR=browser-kling
 */
export class KlingBrowserGenerator implements VideoGeneratorPlugin {
  name = 'browser-kling'
  ...
  async submitJob({ idea }: { idea: Idea; referenceAssets?: string[] }): Promise<{ jobId: string }> {
    ...
    if (child.pid) {
      ...
      console.log(`[kling] Job ${jobId} started - PID ${child.pid} - log: ${logFile}`)
    } else {
      writeJob({ id: jobId, prompt, status: 'failed', error: 'Failed to spawn', startedAt: Date.now() })
      throw new Error('Failed to spawn Kling automation process')
    }
  ...
  async pollJobStatus(jobId: string): Promise<{ status: 'pending' | 'processing' | 'complete' | 'failed'; fileUrl?: string; error?: string }> {
    ...
    if ((job.status === 'pending' || job.status === 'processing') && Date.now() - job.startedAt > 8 * 60 * 1000) {
      return { status: 'failed', error: 'Timed out after 8 minutes' }
    }
