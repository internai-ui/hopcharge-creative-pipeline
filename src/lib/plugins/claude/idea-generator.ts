...
The "Key product facts" sentence above is the ONLY source of truth for anything that reads as a factual claim (pricing, timing, guarantees, partnerships, coverage area). Never invent a new number, statistic, guarantee, or comparison that isn't stated there - no "50% cheaper than petrol", no made-up wait times, no fabricated review counts or customer totals. Benefit language ("no more range anxiety", "charging comes to you") is fine; a specific figure that isn't in the facts above is not. If a funnel stage calls for urgency or proof, create it honestly from tone and framing (e.g. "This week only" only if genuinely an offer being run, not a fabricated deadline) - do not paper over the absence of a real stat with an invented one.
...
Secondary van (for ideas featuring the larger fleet): white cube-shaped mobile battery box body on a light truck chassis - large rectangular white box with red border trim on all panel edges and yellow corner accent pieces, plain unlabelled white panels with no logo or lettering (brand marks are composited on afterwards - never describe a wordmark or text on the van), rear double doors from which a thick charging cable extends to the customer's EV. Yellow Hopcharge A-frame caution sign placed nearby.
...
## Charging cable rule (critical - read before writing imageVisual / videoFirstFrame / videoVisual)
The single most common accuracy failure in generated Hopcharge visuals is the charging cable: it ends up trailing toward a person's hand, running off-frame to an "unseen" car, or just dangling with no visible connection. Whenever the cable appears anywhere in a shot, BOTH ends must be explicit and in-frame - one at the Hopcharge van's charge point, the other seated in the customer's specific EV's charge port - and the customer's EV must be described with the SAME concrete visual detail as the van (its model, colour, and exact position relative to the van), not a passing mention like "her car" - an under-described vehicle is what gets dropped from the generated frame. Never write a cable trailing toward a person, held in a hand, running off-frame, or connecting to an unseen/unnamed vehicle. If a shot's composition genuinely has no room for both the van and the customer's EV together, do not show the cable's free end at all - show it coiled at the van before connection, or already retracted after - never mid-air with nothing on the other end.
...
## Output format - YouTube video copy field rules
The creative is also posted to YouTube as an organic Short / video (not a paid ad). These fields become the video's title + description:
- ytHeadlines: an array of 3-5 DISTINCT short headlines, each <=40 chars, punchy and benefit-led. The FIRST one is used as the video TITLE, so make it strong and specific on its own (do not just repeat the Meta headline).
- ytDescriptions: an array of 2-4 DISTINCT lines, each <=90 chars, expanding the benefit/offer in conversational Indian-English, funnel-appropriate. These fill the video's description box.
- ytCallToAction: exactly ONE of LEARN_MORE, SIGN_UP, GET_QUOTE, SUBSCRIBE, VISIT_SITE, SHOP_NOW - it becomes a short call-to-action line above the link in the description. Pick by funnel (TOF -> LEARN_MORE, MOF -> GET_QUOTE or VISIT_SITE, BOF -> SIGN_UP or SUBSCRIBE).

## Output format - imageVisual field rules
Write imageVisual as a concrete image prompt a generative AI (Flux / Stable Diffusion) can execute directly. Flux reads natural language, not tag soup: write ONE flowing descriptive passage in plain sentences, front-loading the subject in the first clause - do NOT output the bracket labels below literally or write it as a comma-separated tag list. Cover, in this order, as connected prose: [SUBJECT - what the frame is actually about: if a customer is present it is Sara (use her full description: Indian woman mid-to-late 20s, warm wheatish skin, round face full cheeks, large dark almond eyes, full lips warm genuine smile, long extremely thick voluminous near-black wavy hair falling to mid-back, athletic-curvy 165 cm, confident posture) with exact pose and expression - OR, for a no-customer ad, the van, the EV charging alone, a macro connector/charge-port detail, the technician, a cityscape, or an infographic scene] + [VAN - if in frame, use the physical description above and specify which van] + [COMPOSITION - choose and vary: rule of thirds / centred / diagonal / overhead flat-lay / extreme close-up / wide establishing; name foreground / midground / background layers] + [FROZEN MOMENT - the single decisive action] + [SETTING - a SPECIFIC Delhi-NCR location, deliberately different from the other ideas in this batch] + [LIGHTING - a named direction, quality and colour temperature plus TIME OF DAY, e.g. "hard overhead 2pm sun" or "cool blue-hour dusk with warm shopfront practicals" - never a bare adjective like "good lighting" or "nice light", and vary across the batch, not always golden hour] + [LENS - focal length and depth of field - vary the shot type, not always 50 mm] + [MOOD - one adjective]. Do NOT describe text, logos, or overlays - only the visual scene.
...
Write videoVisual as a shot-by-shot description a video AI (Kling / Runway / Higgsfield DOP) can execute. These models are literal, not intuitive: a vague verb ("camera moves") or a bare mood word ("good lighting") produces inconsistent or glitchy motion, while a named move with a stated end state gets executed reliably. So EVERY beat below must name exactly ONE camera move (static lock-off, slow push, pull, pan, tilt, crane rise, dolly, tracking, handheld follow) AND the frame it settles on (e.g. "slow push in, settling close on her hand plugging in the cable" - not just "push in"), plus a concretely-named light source/direction/colour temperature (never "nice lighting" or "cinematic light" alone). If the spot features the customer, that character is Sara (Indian woman mid-to-late 20s, warm wheatish skin, round face full cheeks, large dark almond eyes, long extremely thick voluminous near-black wavy hair to mid-back, athletic-curvy build) - but not every spot needs her; some can be product/van-led, detail-led, or city/lifestyle montages, and any other people (technician, bystanders, other owners) are not Sara and may vary. Structure: [OPENING SHOT 0-3s - hook frame: camera type + subject (Sara if a customer is shown) + composition + the one camera move and its end frame] → [INCITING MOMENT 3-8s - problem or trigger: one camera move + end frame] → [RESOLUTION 8-20s - van arrives / cable connects / EV charges: include van physical description, the subject's action, one camera move + end frame] → [PAYOFF 20-27s - subject calm/relieved (Sara if present), EV charged indicator lit: one camera move + end frame] → [CLOSING FRAME 27-30s - van + EV parked, static beauty shot, named light source and time of day - vary across the batch, not always golden hour]. Use specific camera vocabulary: WES (wide establishing shot), MS (medium shot), CU (close-up), ECU (extreme close-up), OTS (over-the-shoulder). Be concrete - no abstract moods, only physical actions, named light, and camera moves with endpoints.
...
${performanceContext.topPerformers.length > 0 ? `Top ads (Meta: lower CPL is better. YouTube: higher engagement rate is better - it's organic, there is no CPL): ${performanceContext.topPerformers.map(p => p.metric === 'cpl' ? `"${p.idea.title}" (Meta, CPL ₹${p.metricValue.toFixed(0)})` : `"${p.idea.title}" (YouTube, ${p.metricValue.toFixed(1)}% engagement)`).join(', ')}` : ''}
