    ...
    <aside className="w-56 shrink-0 flex flex-col h-screen sticky top-0 bg-white border-r border-brand-border">
      ...
      {user && (
        <div className="border-t border-brand-border px-3 py-3">
          <div className="px-2 pb-1.5 text-xs text-brand-muted truncate" title={user.email}>
            {user.name || user.email}
          </div>
          <form action="/api/auth/signout" method="post">
            <button
              type="submit"
              className="flex w-full items-center gap-3 px-3 py-2 rounded-lg text-sm text-brand-muted hover:text-brand-dark hover:bg-brand-surface transition-all duration-200"
            >
              <SignOutIcon />
              Sign out
            </button>
